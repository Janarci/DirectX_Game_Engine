...
Plane::Plane(string name, void* shaderByteCode, size_t sizeShader) :AGameObject(name)
{
	...
	this->constantBuffer->load(&data, sizeof(data));
...
void Plane::draw(int width, int height, VertexShader* vertexShader, PixelShader* pixelShader, ConstantBuffer* constantBuffer, Camera* cam)
{
	...
	cbData.worldMatrix.setScale(Vector3D(3, 3, 1));
